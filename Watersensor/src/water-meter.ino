...
int magic_code_box(int16_t sen_a, int16_t sen_b, int16_t sen_c) {
    static bool started_water_usaged = false; // if start of pulse detected ignore liters overcommunicating
    if (not_inited) {
        state.phase  = 0;
        state.fine   = 0;
        state.liters = 0;
    ...
    // * If real start not happend assume calculation are fake intel
    if(state.c_min+20 > state.c_max and started_water_usaged == false){
        mili_liters_total = 0;
    } else{
        started_water_usaged = true;
    }
...
void setup() {
    ...
    Serial.begin(115200);
    Serial.print("Water sensor V2.0.1!");
